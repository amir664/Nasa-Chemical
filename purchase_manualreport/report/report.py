...
class CustomReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if product_ids:
            where_clauses.append("pt.id = ANY(%s)")
            params.append(product_ids)  # Corrected to use ANY()

        if vendor_ids:
            where_clauses.append("rs.id = ANY(%s)")
        ...
        cr.execute(query, tuple(params))  # Ensure params is passed as a tuple
